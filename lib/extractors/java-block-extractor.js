...
module.exports = function (code, blockName) {
  ...
  for (var i = lines.length - 1; i >= 0; i--) {
    ...
    if (javadocStart !== -1) {
      ...
      if (lines[lines.length - 1].search(/\*\/\s*$/) === -1) {
        console.warn(chalk.yellow('W base-java-block-extractor: Javadoc comment didn\'t end before ' + blockName));
        return classBlock;
      }
  ...
  console.warn(chalk.yellow('W base-java-block-extractor: No javadoc found for ' + blockName));
  return classBlock;
};
