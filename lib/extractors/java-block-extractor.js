...
/**
 * Extractor a Java class via block-extractor and includes a javadoc comment
...
module.exports = function (code, blockName) {
  ...
  if (beforeClassBlock.replace(/\s/g, '').length === 0) {
    console.warn('W java-block-extractor: ' + blockName + ' is at beginning of file');
  }
  ...
  for (var i = lines.length - 1; i >= 0; i--) {
    ...
    if (javadocStart !== -1) {
      ...
      for (var j = 0; j < lines.length - 1; j++) {
        if (lines[j].search(/\*\//) !== -1) {
          // console.warn(chalk.yellow('W Javadoc comment ended early for class ' + className + ' using javaClassExtractor'));
          return classBlock;
        }
      }
      if (lines[lines.length - 1].search(/\*\/\s*$/) === -1) {
        console.warn(chalk.yellow('W java-block-extractor: Javadoc comment didn\'t end before ' + blockName));
        return classBlock;
      }
  ...
  console.warn(chalk.yellow('W java-block-extractor: No javadoc found for ' + blockName));
  return classBlock;
};
